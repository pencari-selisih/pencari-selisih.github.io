    ...
    function setScanUIGating(isRunning) {
        try {
            ...
            if (isRunning) {
                ...
                // Disable toggle MULTICHAIN dan MANY CEX di modal filter scanner saat scan berjalan (CEX/MultiCEX mode)
                const isCEXRunning = (String(mode.type || '').toLowerCase() === 'cex');
                if (isCEXRunning) {
                    $('#modal-filter-multichain, #modal-filter-manycex').prop('disabled', true)
                        .closest('label').css({ opacity: 0.45, pointerEvents: 'none', cursor: 'not-allowed' });
                }
            } else {
                // Re-enable toolbar
                ...
                // Re-enable toggle MULTICHAIN dan MANY CEX di modal filter scanner
                $('#modal-filter-multichain, #modal-filter-manycex').prop('disabled', false)
                    .closest('label').css({ opacity: '', pointerEvents: '', cursor: '' });
            }
        } catch (_) { /* noop */ }
    }
